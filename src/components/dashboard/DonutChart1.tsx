...
export default function GamesPlayedDonut({
  playedByGamesName
}: {
  playedByGamesName: { gameName: string | null; count: number }[]
}) {
  const chartData = React.useMemo(
    () =>
      playedByGamesName.map((row, i) => ({
        game: row.gameName ?? "Unknown",
  ...
  return (
    <Card className="flex flex-col w-[45%]">
      <CardHeader className="items-center pb-0">
        ...
        <CardDescription>By Game Name</CardDescription>
      </CardHeader>
